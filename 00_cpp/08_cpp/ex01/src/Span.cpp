...
/*   Updated: 2025/07/26 16:10:38 by jfranco          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Span::Span(unsigned int size)
	: _size(size), _MaxDistance(0), _MinDistance(0), _InterIndex(0)
{
...
void	Span::searchSpan( void )
{
	for (size_t i = 0; i < _InterIndex; ++i)
	{
		...
		{
			long long diff = std::llabs((long long)this->_array[i] - (long long)this->_array[y]);
			if ( diff > static_cast<long long>(_MaxDistance))
			{
				_MaxDistance = diff;
			}
			if ((_MaxDistance < _MinDistance) || _MinDistance == 0)
			{
